  ...
  start)
    ...
    then
      ...
      then
        ...
        echo "JETTY_USER must be set"
        exit 1
      fi

      touch "$JETTY_LOGS/stdout.log"
      chown "$JETTY_USER:$JETTY_USER" "$JETTY_LOGS/stdout.log"
      start-stop-daemon -C -S -p"$JETTY_PID" $CH_USER -d"$JETTY_BASE" -b -m -a "$JAVA" -- "${RUN_ARGS[@]}" start-log-file="$JETTY_LOGS/start.log" &>> "$JETTY_LOGS/stdout.log"
